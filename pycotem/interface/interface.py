...
def get_direction():
	...
	if s<3: 
		...
		Tp=np.zeros((3,np.shape(tilt_a)[0]))
...
def open_image():
    ...
    a.figure.clear()
    a=figure.add_subplot(111)
    image_diff=QtGui.QFileDialog.getOpenFileName(PyInterface,"Open image file", "", "*.png *.jpg *.bmp *.tiff *.tif *.jpeg")
    img=Image.open(str(image_diff))
    img=np.array(img)
    a.imshow(img,origin='upper')
    figure.suptitle(str(image_diff))
